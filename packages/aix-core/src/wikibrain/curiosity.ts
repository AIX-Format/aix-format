...
export class CuriosityEngine {
  private static MAX_CURIOSITY = 10;

  /**
   * Feed curiosity from self-review score (RULE 7)
   */
  static async feed(agentId: string, selfScore: number, taskType: string): Promise<number> {
    const currentScore = await this.getCuriosityScore(agentId);
    
    // Inverse relationship: poor performance triggers higher curiosity/learning drive
    const boost = (1 - selfScore) * 5; 
    const newScore = Math.min(1000, currentScore + boost);

    await kv.set(KEYS.agentCuriosityScore(agentId), newScore);

    if (boost > 2) {
      const bus = getBus();
      await bus.emitEvent('agent:curiosity_spike', agentId, {
        score: newScore,
        boost,
        focus: taskType,
        message: "High curiosity spike detected due to performance gap."
      });
    }
